  ...
  PhotosBloc() : super(PhotosIntialState()) {
    on<SetInitialPhotos>((event, emit) async {
      ...
      for(int i=0; i<cat.length; i++){
        String catUrl ="https://api.pexels.com/v1/search?query=${cat[i]}&per_page=20";
      ...
      if(res.statusCode == 200){
        ...
        emit(PhotosLoadedState(photos: PhotosDataModel.fromJson(data),catPhotos: catPhotos));
      }else{
       emit( PhotosErrorState(error: "Photos Could not load : ${res.statusCode}"));
      }
    });
    on<SearchPhoto>((event,emit)async{
      emit(PhotosLoadingState());
      String catUrl ="https://api.pexels.com/v1/search?query=${event.search}&per_page=20";
